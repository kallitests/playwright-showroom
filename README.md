...
| 🛠️ **Admin scripts** | Python utilities for Git workflow, GitHub auth, and environment validation |
...
│
├── scripts/                        # Standalone Python admin utilities
│   ├── README.md                   # Scripts overview and quick start
│   ├── git-commit-push/            # Hardened git add → commit → push
│   ├── git-commit-push-ai/         # Same + AI commit message via LangChain
│   ├── update-github-username/     # Bulk remote URL updater
│   ├── env-check/                  # Pre-launch environment validator
│   └── github-auth-setup/          # GitHub SSH / PAT auth wizard
│
...
Standalone Python utilities for Git workflow, GitHub authentication, and environment validation. Each script lives in [`scripts/`](scripts/) with its own README.

| Script | Purpose | AI |
|---|---|---|
| [`git-commit-push/`](scripts/git-commit-push/) | Hardened `add → commit → push` with pre-flight checks and secret detection | — |
| [`git-commit-push-ai/`](scripts/git-commit-push-ai/) | Same + commit message generated by Claude or Ollama from the staged diff | LangChain |
| [`update-github-username/`](scripts/update-github-username/) | Bulk-update GitHub remote URLs across all local repos after a username change | — |
| [`env-check/`](scripts/env-check/) | Pre-launch validator — `.env`, packages, network, Docker, Playwright browsers | — |
| [`github-auth-setup/`](scripts/github-auth-setup/) | Interactive wizard to fix SSH / PAT GitHub authentication errors | — |

```bash
# Secure commit & push — manual message
python scripts/git-commit-push/git_commit_push.py

# Secure commit & push — AI message via Ollama (local, free)
python scripts/git-commit-push-ai/git_commit_push_ai.py

# Secure commit & push — AI message via Claude
...
python scripts/update-github-username/update_github_username.py old_user new_user
```

All scripts are standard library only (except `git-commit-push-ai` which optionally uses `langchain-anthropic` or `langchain-ollama`).
