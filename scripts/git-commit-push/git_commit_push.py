...
NOTE : la connexion SSH persistante (ControlMaster/ControlPersist) est
DÉSACTIVÉE par défaut — elle générait des avertissements bruyants sur
Git Bash / Windows ("mux_client_request_session... Connection reset by
peer") sans bénéfice net. Le code est conservé (commenté) dans
ensure_persistent_ssh_config() / check_github_ssh_identity() et dans
main() si besoin de la réactiver sur un autre environnement.
...
def git_push(repo_path: str, branch: str) -> None:
    ...
    if "set-upstream" in combined or "no upstream branch" in combined:
        ...
        if denied_user and denied_user in ALLOWED_GITHUB_USERS:
            ...
            # Le refus est donc probablement transitoire — agent qui vient
            # de changer de clé, etc. On retente une fois avant d'abandonner.
            warn(
                f"Push refusé pour '{denied_user}', mais ce compte est un\n"
                f"  collaborateur connu de ce dépôt ({', '.join(ALLOWED_GITHUB_USERS)}).\n"
                "  Nouvelle tentative…"
            )
            # ensure_persistent_ssh_config()  # DÉSACTIVÉ — voir note en tête de fichier
            ...
            abort(
                ...
                "  Vérifiez ses droits d'accès directement sur GitHub\n"
                "  (Settings → Collaborators) ou relancez github_auth_setup.py."
            )
        else:
            warn(
...
def main() -> None:
    ...
    # DÉSACTIVÉ : ControlMaster/ControlPersist génère des avertissements
    # bruyants ("mux_client_request_session... Connection reset by peer")
    # sur Git Bash / Windows sans réel bénéfice ici. Fonctions conservées
    # plus bas si on veut réactiver un jour sur un autre environnement.
